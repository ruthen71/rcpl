...
// 直線 l, 点 p の交差判定
template <class T>
inline bool is_intersect(const Line<T>& l, const Point<T>& p) {
...
// 線分 s, 点 p の交差判定
template <class T>
inline bool is_intersect(const Segment<T>& s, const Point<T>& p) {
...
template <class T> bool is_intersect(const Line<T>& l, const Segment<T>& s) {
    // s.a と s.b が直線 l に関して同じ側 (直線上を除き)
    // にある場合に限り交差しない
...
template <class T>
inline bool is_intersect(const Segment<T>& s1, const Segment<T>& s2) {
    auto c1 = ccw(s1.a, s1.b, s2.a);
    auto c2 = ccw(s1.a, s1.b, s2.b);
    auto c3 = ccw(s2.a, s2.b, s1.a);
    auto c4 = ccw(s2.a, s2.b, s1.b);
    // 平行な場合: 重なるなら 1
    // 次元で考えると必ず端点のどれかがもう一方の線分上にある
    if (c1 == Ccw::ON_SEGMENT or c2 == Ccw::ON_SEGMENT or
        c3 == Ccw::ON_SEGMENT or c4 == Ccw::ON_SEGMENT)
        return true;
    // 平行でない場合: 一方の線分の両側にもう一方の線分の端点がある,
    // を線分を入れ替えても成立 平行だが重ならない場合は以下の条件は成立しない
    bool ok1 = ((c1 == Ccw::CLOCKWISE and c2 == Ccw::COUNTER_CLOCKWISE) or
                (c1 == Ccw::COUNTER_CLOCKWISE and c2 == Ccw::CLOCKWISE));
    bool ok2 = ((c3 == Ccw::CLOCKWISE and c4 == Ccw::COUNTER_CLOCKWISE) or
...
// 円 c1, c2 の交差判定
template <class T>
inline bool is_intersect(const Circle<T>& c1, const Circle<T>& c2) {
...
// 円 c, 点 p の交差判定
template <class T>
inline bool is_intersect(const Circle<T>& c, const Point<T>& p) {
    return equal(norm(p - c.o), c.r * c.r);
}
template <class T>
inline bool is_intersect(const Point<T>& p, const Circle<T>& c) {
    return equal(norm(p - c.o), c.r * c.r);
}

// 円 c, 直線 l の交差判定
template <class T>
inline bool is_intersect(const Circle<T>& c, const Line<T>& l) {
...
template <class T>
inline bool is_intersect(const Line<T>& l, const Circle<T>& c) {
    return is_intersect(c, l);
}
