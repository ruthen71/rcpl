...
int main() {
    ...
    for (int qq = 0; qq < Q; qq++) {
        ...
        if (type == 1) {
            ...
            l[qq]--;
            auto nodes =
                enumerate_segment_tree_range_covering_nodes(size, l[qq], r[qq]);
            for (auto&& i : nodes) d[i].push(x[qq]);
        } else if (type == 2) {
            std::cin >> id[qq];
            id[qq]--;
            auto nodes = enumerate_segment_tree_range_covering_nodes(
                size, l[id[qq]], r[id[qq]]);
            for (auto&& i : nodes) d[i].erase(x[id[qq]]);
        } else {
            std::cin >> id[qq];
            id[qq]--;
            auto nodes =
                enumerate_segment_tree_point_containing_nodes(size, id[qq]);
            int mx = 0;
            for (auto&& i : nodes) {
